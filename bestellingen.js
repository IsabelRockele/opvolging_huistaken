// ==============================================
// BESTELLINGEN - automatische klas/graadgroep versie
// ==============================================
...
// ==============================================
// BESTELLINGEN: automatische indeling
// ==============================================
const GROEPEN = [
...
const BESTELLINGEN_PREFIX = 'bestellingen_';
const BESTELLINGEN_COLLECTION = 'klasbeheer_teams';
let rol = '';
let isSecretariaatGebruiker = false;
let toegelatenKlassen = [];
let klasLeerkrachtDocs = [];
...
function groepVoorKlas(klas) {
  return GROEPEN.find(g => g.klassen.includes(String(klas || '').trim()));
}

function leerjaarVoorKlas(klas) {
  if (!actieveGroep) return 'L1';
  return actieveGroep.klassen[0] === klas ? 'L1' : 'L2';
}

function docIdVoorGroep(groep) {
  return BESTELLINGEN_PREFIX + (teamData?.instellingen?.schooljaar || '2026-2027') + '_' + groep.id;
}

function bestellingenDocRef(groep = actieveGroep) {
  return doc(db, BESTELLINGEN_COLLECTION, docIdVoorGroep(groep));
}

function isSchoolBreedBestellingen() {
  return ['directie', 'zorgcoordinator', 'zorgleerkracht', 'secretariaat'].includes(rol);
}

function isSecretariaatBestellingen() {
  return ['directie', 'secretariaat'].includes(rol);
}

function toonBestellingenFout(titel, boodschap, err) {
  console.error(titel, err || boodschap);
  const loading = document.getElementById('loading-scherm');
  const login = document.getElementById('login-scherm');
  const team = document.getElementById('team-scherm');
  const app = document.getElementById('app-scherm');
  ...
  if (team) {
    ...
    team.innerHTML =
      '<h2>' + escape(titel || 'Bestellingen kon niet laden') + '</h2>' +
      '<p>' + escape(boodschap || 'Er liep iets mis bij het openen van Bestellingen.') + '</p>' +
      (detail ? '<p style="font-size:0.9em;color:#6b655d;">Foutmelding: ' + escape(detail) + '</p>' : '') +
      '<p style="font-size:0.9em;color:#6b655d;">Vernieuw de pagina met Ctrl+F5. Blijft dit staan, stuur deze foutmelding door.</p>';
  } else {
    alert((titel || 'Bestellingen kon niet laden') + (err && err.message ? '\n\n' + err.message : ''));
  }
}
...
onAuthStateChanged(auth, async (user) => {
  try {
    ...
    toonBestellingenFout(
      'Bestellingen kon niet starten',
      'Er liep iets mis net na het aanmelden.',
      err
    );
  }
});
...
async function opzoekenTeam() {
  ...
  try {
    await laadBestellingenRolEnKlassen();
    const groepen = groepenVoorBestellingenUser();

    if (!groepen.length) {
      tooSchermen({ loading: false, login: false, team: true, app: false });
      const teamScherm = document.getElementById('team-scherm');
      if (teamScherm) {
        teamScherm.innerHTML = '<h2>Geen klas gevonden</h2><p>Je account is nog niet gekoppeld aan een klas in de collectie klasleerkrachten.</p><p>Vraag aan secretariaat of ICT om je e-mailadres bij je klas te zetten.</p>';
      }
      return;
    }

    actieveGroep = groepen[0];
    actieveKlas = isSecretariaatGebruiker
      ? actieveGroep.klassen[0]
      : (actieveGroep.klassen.find(k => toegelatenKlassen.includes(k)) || actieveGroep.klassen[0]);

    await openBestellingenGroep(actieveGroep.id, actieveKlas);
  } catch (err) {
    console.error('Bestellingen starten mislukt:', err);
    alert('Bestellingen kon niet starten: ' + err.message);
    tooSchermen({ loading: false, login: false, team: true, app: false });
  }
}

async function laadBestellingenRolEnKlassen() {
  const rolSnap = await getDoc(doc(db, 'schoolrollen', huidigeUser.uid));
  rol = rolSnap.exists() ? String(rolSnap.data().rol || '').toLowerCase() : '';
  isSecretariaatGebruiker = rol === 'secretariaat';

  if (isSchoolBreedBestellingen()) {
    toegelatenKlassen = alleKlassen();
  } else {
    const email = (huidigeUser.email || '').toLowerCase();
    const uidQuery = query(collection(db, 'klasleerkrachten'), where('leerkracht_uids', 'array-contains', huidigeUser.uid));
    ...
    snaps.forEach(snap => snap.docs.forEach(d => {
      const item = d.data() || {};
      klasLeerkrachtDocs.push(item);
      if (item.klas) found.add(String(item.klas).trim());
    }));
    toegelatenKlassen = [...found];
  }
}

function groepenVoorBestellingenUser() {
  if (isSchoolBreedBestellingen()) return GROEPEN;
  return GROEPEN.filter(g => g.klassen.some(k => toegelatenKlassen.includes(k)));
}

async function openBestellingenGroep(groepId, klas) {
  const groep = GROEPEN.find(g => g.id === groepId);
  if (!groep) return;
  actieveGroep = groep;
  actieveKlas = klas || groep.klassen[0];
  huidigLeerjaar = leerjaarVoorKlas(actieveKlas);
  teamId = docIdVoorGroep(groep);

  try {
    await zorgDatBestellingenDocumentBestaat(groep);
  } catch (err) {
    toonBestellingenFout(
      'Bestellingen kan de data niet klaarzetten',
      'Firebase weigert het aanmaken of openen van het bestellingen-document. Waarschijnlijk moeten de Firestore-regels deze nieuwe bestellingen-documenten nog toelaten.',
      err
    );
    return;
  }
...
async function zorgDatBestellingenDocumentBestaat(groep) {
  const ref = bestellingenDocRef(groep);

  try {
    const snap = await getDoc(ref);
    if (snap.exists()) return;
  } catch (err) {
    console.warn('Bestellingen-document kon niet gelezen worden; we proberen het eerst aan te maken.', err);
  }

  const startdata = await startDataVoorBestellingenGroep(groep);
  await setDoc(ref, startdata, { merge: true });
}
// Realtime listener koppelen aan team-doc
function koppelRealtime() {
  if (teamUnsubscribe) teamUnsubscribe();

  const ref = bestellingenDocRef(actieveGroep);
  teamUnsubscribe = onSnapshot(ref, (snap) => {
    (async () => {
      try {
        if (!snap.exists()) {
          toonBestellingenFout(
            'Bestellingen-document ontbreekt nog',
            'De app kon het document niet vinden nadat het klaargezet werd.',
            null
          );
          return;
        }
        ...
        teamData = snap.data();
        normaliseerTeamData();
        zorgDatGroepVeldenKloppen();
        tooSchermen({ loading: false, login: false, team: false, app: true });
        allesRenderen();
        zetSyncStatus('Bewaard in cloud', false);
      } catch (err) {
        toonBestellingenFout(
          'Bestellingen kon niet klaargezet worden',
          'De app is aangemeld, maar het openen van de bestellingsdata lukt nog niet.',
          err
        );
      }
    })();
    ...
    toonBestellingenFout(
      'Bestellingen heeft geen toegang tot Firebase',
      'De verbinding met de cloudgegevens wordt tegengehouden.',
      err
    );
    zetSyncStatus('Verbinding verloren', true);
  });
}
async function startDataVoorBestellingenGroep(groep) {
  let startdata = standaardData(huidigeUser.uid, huidigeUser.email || '');

  if (groep.id === 'graad1') {
    try {
      const oud = await zoekOudKlasbeheerTeam();
      if (oud) {
        startdata = {
          ...startdata,
          instellingen: oud.instellingen || startdata.instellingen,
          werkboeken: oud.werkboeken || startdata.werkboeken,
          budget: oud.budget || 0,
          bestelKinderen: oud.bestelKinderen || startdata.bestelKinderen,
          lyreco: Array.isArray(oud.lyreco) ? oud.lyreco : [],
          actions: Array.isArray(oud.actions) ? oud.actions : [],
          knutsels: Array.isArray(oud.knutsels) ? oud.knutsels : [],
          stockCats: Array.isArray(oud.stockCats) ? oud.stockCats : startdata.stockCats,
          archief: Array.isArray(oud.archief) ? oud.archief : []
        };
        startdata.migratieVanOudKlasbeheer = true;
      }
    } catch (err) {
      console.warn('Oude klasbeheerdata kon niet automatisch gelezen worden. Blanco startdata wordt gebruikt.', err);
    }
  }

  startdata.bestellingenApp = true;
  startdata.groepId = groep.id;
  startdata.groepNaam = groep.naam;
  startdata.eigenaar = huidigeUser.uid;
  const leden = ledenVoorGroep(groep);
  startdata.leden_uids = leden.uids;
  startdata.leden_emails = leden.emails;
  startdata.laatst_gewijzigd = serverTimestamp();
  startdata.aangemaakt = serverTimestamp();

  if (!startdata.instellingen) startdata.instellingen = {};
  if (!startdata.instellingen.L1) startdata.instellingen.L1 = {};
  if (!startdata.instellingen.L2) startdata.instellingen.L2 = {};
  startdata.instellingen.L1.klasNr = groep.klassen[0];
  startdata.instellingen.L2.klasNr = groep.klassen[1];
  if (!startdata.instellingen.L1.naam) startdata.instellingen.L1.naam = groep.klassen[0];
  if (!startdata.instellingen.L2.naam) startdata.instellingen.L2.naam = groep.klassen[1];
  return startdata;
}
function ledenVoorGroep(groep) {
  const uids = new Set([huidigeUser.uid]);
  const emails = new Set([(huidigeUser.email || '').toLowerCase()].filter(Boolean));
  klasLeerkrachtDocs
    .filter(item => groep.klassen.includes(String(item.klas || '').trim()))
    .forEach(item => {
      (item.leerkracht_uids || []).forEach(uid => uid && uids.add(uid));
      (item.leerkracht_emails || []).forEach(mail => mail && emails.add(String(mail).toLowerCase()));
    });
  return { uids: [...uids], emails: [...emails] };
}

async function zoekOudKlasbeheerTeam() {
  if (!huidigeUser || isSecretariaatGebruiker) return null;
  const email = (huidigeUser.email || '').toLowerCase();
  const uidQuery = query(collection(db, 'klasbeheer_teams'), where('leden_uids', 'array-contains', huidigeUser.uid));
  const emailQuery = query(collection(db, 'klasbeheer_teams'), where('leden_emails', 'array-contains', email));
  const snaps = await Promise.all([getDocs(uidQuery), getDocs(emailQuery)]);
  for (const snap of snaps) {
    for (const d of snap.docs) {
      if (!d.id.startsWith(BESTELLINGEN_PREFIX)) return d.data();
    }
  }
  return null;
}

function zorgDatGroepVeldenKloppen() {
  if (!teamData || !actieveGroep) return;
  teamData.bestellingenApp = true;
  teamData.groepId = actieveGroep.id;
  teamData.groepNaam = actieveGroep.naam;
  if (!teamData.instellingen) teamData.instellingen = {};
  if (!teamData.instellingen.L1) teamData.instellingen.L1 = {};
  if (!teamData.instellingen.L2) teamData.instellingen.L2 = {};
  teamData.instellingen.L1.klasNr = actieveGroep.klassen[0];
  teamData.instellingen.L2.klasNr = actieveGroep.klassen[1];
}
function normaliseerTeamData() {
  ...
  if (!teamData.bestellingenMeldingen.werkboekenPerKlas) teamData.bestellingenMeldingen.werkboekenPerKlas = {};
  const klassenVoorMeldingen = actieveGroep ? actieveGroep.klassen : alleKlassen();
  klassenVoorMeldingen.forEach(klas => {
    if (!teamData.bestellingenMeldingen.werkboekenPerKlas[klas]) {
      teamData.bestellingenMeldingen.werkboekenPerKlas[klas] = { datum: '', tekst: '' };
    }
    if (teamData.bestellingenMeldingen.werkboekenPerKlas[klas].datum === undefined) teamData.bestellingenMeldingen.werkboekenPerKlas[klas].datum = '';
    if (teamData.bestellingenMeldingen.werkboekenPerKlas[klas].tekst === undefined) teamData.bestellingenMeldingen.werkboekenPerKlas[klas].tekst = '';
  });
  if (!teamData.bestellingenMeldingen.lyreco) teamData.bestellingenMeldingen.lyreco = { datum: '', tekst: '' };
  if (teamData.bestellingenMeldingen.lyreco.datum === undefined) teamData.bestellingenMeldingen.lyreco.datum = '';
  if (teamData.bestellingenMeldingen.lyreco.tekst === undefined) teamData.bestellingenMeldingen.lyreco.tekst = '';
  if (teamData.bestellingenMeldingen.lyreco.leverdatum === undefined) teamData.bestellingenMeldingen.lyreco.leverdatum = '';
  if (!teamData.bestellingenMeldingen.werkboekenAlgemeen) {
    const eersteKlas = klassenVoorMeldingen[0] || '';
    const oud = eersteKlas && teamData.bestellingenMeldingen.werkboekenPerKlas
      ? teamData.bestellingenMeldingen.werkboekenPerKlas[eersteKlas]
      : null;
  ...
  if (teamData.bestellingenMeldingen.werkboekenAlgemeen.datum === undefined) teamData.bestellingenMeldingen.werkboekenAlgemeen.datum = '';
  if (teamData.bestellingenMeldingen.werkboekenAlgemeen.tekst === undefined) teamData.bestellingenMeldingen.werkboekenAlgemeen.tekst = '';
...
async function bewaarNu() {
  ...
  try {
    ...
    await setDoc(ref, {
      ...teamData,
      laatst_gewijzigd: serverTimestamp(),
      laatst_gewijzigd_door: huidigeUser?.email || huidigeUser?.uid || ''
    }, { merge: true });
    zetSyncStatus('Bewaard in cloud', false);
  } catch (err) {
    console.error('Bewaarfout:', err);
    zetSyncStatus('Kon niet bewaren', true);
  } finally {
...
function klasVoorLeerjaar(lj = huidigLeerjaar) {
  if (!actieveGroep || !Array.isArray(actieveGroep.klassen)) return lj;
  return lj === 'L1' ? actieveGroep.klassen[0] : actieveGroep.klassen[1];
}

function leerjaarNaamVoorKlas(klas) {
  const match = String(klas || '').match(/\d+/);
  const nr = match ? parseInt(match[0], 10) : 0;
  if (!nr) return '';
  return nr + (nr === 1 ? 'ste' : 'de') + ' leerjaar';
}

function werkboekenLabel(lj = huidigLeerjaar) {
  const klas = klasVoorLeerjaar(lj);
  const leerjaar = leerjaarNaamVoorKlas(klas);
  if (leerjaar && klas) return leerjaar + ' (' + klas + ')';
  return lj === 'L1' ? 'eerste klas van deze graadgroep' : 'tweede klas van deze graadgroep';
}
...
function teamHeaderTekst() {
  ...
  ['L1', 'L2'].forEach(lj => {
    ...
    const klas = (d.klasNr || klasVoorLeerjaar(lj) || '').trim();
    const label = werkboekenLabel(lj);
    let stuk = label + ': ';
    if (naam) stuk += naam;
    if (!naam && klas) stuk += 'klas ' + klas;
    stukjes.push(stuk.trim());
  });
  return stukjes.join(' - ');
}
...
async function laadSecretariaatStatusOverzicht() {
  if (secretariaatStatusAanHetLaden) return;
  const secretariaat = typeof isSecretariaatBestellingen === 'function'
    ? isSecretariaatBestellingen()
    : isSecretariaatGebruiker;
  if (!secretariaat || !teamData) return;

  secretariaatStatusAanHetLaden = true;
  try {
    const schooljaar = teamData.instellingen?.schooljaar || '2026-2027';
    const entries = await Promise.all(GROEPEN.map(async groep => {
      try {
        const ref = doc(db, BESTELLINGEN_COLLECTION, BESTELLINGEN_PREFIX + schooljaar + '_' + groep.id);
        const snap = await getDoc(ref);
        return [groep.id, snap.exists() ? snap.data() : null];
      } catch (err) {
        console.warn('Status laden mislukt voor ' + groep.id, err);
        return [groep.id, null];
      }
    }));
...
function renderWerkboekenStatusOverzicht() {
  const secretariaat = typeof isSecretariaatBestellingen === 'function'
    ? isSecretariaatBestellingen()
    : isSecretariaatGebruiker;
  const groepen = secretariaat ? GROEPEN : (actieveGroep ? [actieveGroep] : []);
...
function renderBestellingenMeldingen() {
        ...
        <div class="deadline-paneel rustig secretariaat-start">
          ...
          <div class="deadline-grid compact">
            ...
            <div style="grid-column:1/-1;">
              ...
              <input data-secretariaat-deadline type="text" id="deadline-ly-tekst" value="${escape(ly.tekst || '')}" placeholder="bv. Plaats je bestelling in Lyreco met deze leverdatum">
            </div>
          </div>
          <div class="deadline-acties">
      ...
      wbBox.innerHTML = `
        ...
        <div class="leerkracht-status ${status.inOrde ? 'klaar' : 'niet-klaar'}">
          <label>
            ...
            Werkboeken van ${escape(actieveKlas)} zijn aangepast en in orde
          </label>
  ...
  if (lyBox) {
    lyBox.innerHTML = secretariaat ? '' : meldingRegelHtml('Lyreco-bestelling', ly);
  }
}

window.bestellingenMeldingenOpslaan = function () {
  const secretariaat = typeof isSecretariaatBestellingen === 'function'
    ? isSecretariaatBestellingen()
    : isSecretariaatGebruiker;
  ...
  teamData.bestellingenMeldingen.lyreco = {
    datum: document.getElementById('deadline-ly-datum')?.value || '',
    leverdatum: document.getElementById('deadline-ly-leverdatum')?.value || '',
    tekst: document.getElementById('deadline-ly-tekst')?.value.trim() || ''
  };
  planBewaren();
...
// RENDER ALLES
// ==============================================
function werkboekenLeerjaarKnoppenBijwerken() {
  const knopL1 = document.querySelector('.leerjaar-knop[data-leerjaar="L1"]');
  const knopL2 = document.querySelector('.leerjaar-knop[data-leerjaar="L2"]');
  if (knopL1) knopL1.textContent = werkboekenLabel('L1');
  if (knopL2) knopL2.textContent = werkboekenLabel('L2');
}

function werkboekenPerKlasUiToepassen() {
  const schakelaar = document.querySelector('.leerjaar-schakelaar');
  if (schakelaar) schakelaar.style.display = 'none';

  const titel = document.querySelector('#paneel-werkboeken h2');
  if (titel) titel.textContent = 'Werkboeken per klas';

  const sub = document.querySelector('#paneel-werkboeken .sub');
  if (sub) {
    sub.innerHTML = 'Kies bovenaan de klas. Je ziet en bewerkt hier alleen de werkboeken van die ene klas. Secretariaat kan op dezelfde manier per klas meekijken en deadlines zetten.';
  }
}

function secretariaatKeuzeUiToepassen() {
...
function allesRenderen() {
  ...
  stockTonen();
  ledenTonen();
  pasSecretariaatReadonlyToe();
}

function renderBestellingenKeuzes() {
  const balk = document.querySelector('.team-balk');
  if (!balk || document.getElementById('bestellingen-keuzes')) return;
  const wrap = document.createElement('span');
  wrap.id = 'bestellingen-keuzes';
  wrap.style.display = 'inline-flex';
  wrap.style.gap = '8px';
  wrap.style.alignItems = 'center';
  wrap.style.flexWrap = 'wrap';
  wrap.innerHTML =
    '<label style="margin:0;font-weight:700;">Graadgroep</label>' +
    '<select id="bestellingen-groep-wissel" style="width:auto;min-width:130px;"></select>' +
    '<label style="margin:0;font-weight:700;">Klas</label>' +
    '<select id="bestellingen-klas-wissel" style="width:auto;min-width:80px;"></select>';
  const sync = document.getElementById('sync-status');
  balk.insertBefore(wrap, sync || null);

  document.getElementById('bestellingen-groep-wissel').addEventListener('change', e => {
    const groep = GROEPEN.find(g => g.id === e.target.value);
    const klas = isSecretariaatGebruiker
      ? groep.klassen[0]
      : (groep.klassen.find(k => toegelatenKlassen.includes(k)) || groep.klassen[0]);
    openBestellingenGroep(groep.id, klas);
  });
  document.getElementById('bestellingen-klas-wissel').addEventListener('change', e => {
    actieveKlas = e.target.value;
    huidigLeerjaar = leerjaarVoorKlas(actieveKlas);
    allesRenderen();
  });
}

function pasBestellingenRolToe() {
  const groepen = groepenVoorBestellingenUser();
  const groepSelect = document.getElementById('bestellingen-groep-wissel');
  const klasSelect = document.getElementById('bestellingen-klas-wissel');
  if (groepSelect) {
    groepSelect.innerHTML = groepen.map(g => '<option value="' + escape(g.id) + '">' + escape(g.naam) + '</option>').join('');
    groepSelect.value = actieveGroep?.id || groepen[0]?.id || '';
    groepSelect.disabled = !isSchoolBreedBestellingen() && groepen.length <= 1;
  }
  if (klasSelect && actieveGroep) {
    const klassen = isSchoolBreedBestellingen() ? actieveGroep.klassen : actieveGroep.klassen.filter(k => toegelatenKlassen.includes(k));
    klasSelect.innerHTML = klassen.map(k => '<option value="' + escape(k) + '">' + escape(k) + '</option>').join('');
    if (!klassen.includes(actieveKlas)) actieveKlas = klassen[0] || actieveGroep.klassen[0];
    klasSelect.value = actieveKlas;
    klasSelect.disabled = klassen.length <= 1;
  }

  const teamNaam = document.getElementById('team-naam');
  if (teamNaam) teamNaam.textContent = actieveGroep ? actieveGroep.naam + ' - ' + actieveKlas : '';
  const userEmail = document.getElementById('user-email');
  if (userEmail && huidigeUser) userEmail.textContent = huidigeUser.email || '';

  document.querySelectorAll('.tab').forEach(tab => {
    const alleenWerkboeken = isSecretariaatGebruiker && tab.dataset.tab !== 'werkboeken';
    tab.style.display = alleenWerkboeken ? 'none' : '';
  });
  if (isSecretariaatGebruiker) {
    document.querySelectorAll('.tab').forEach(x => x.classList.remove('actief'));
    document.querySelectorAll('.paneel').forEach(x => x.classList.remove('actief'));
    document.querySelector('.tab[data-tab="werkboeken"]')?.classList.add('actief');
    document.getElementById('paneel-werkboeken')?.classList.add('actief');
  }
}
...
function klasInstellingenTonen() {
  ...
  if (klasBalkLabel) klasBalkLabel.textContent = werkboekenLabel();
...
window.leerjaarWisselen = function (lj) {
  ...
  if (!klas) return;
  const klasSelect = document.getElementById('bestellingen-klas-wissel');
  if (klasSelect && [...klasSelect.options].some(opt => opt.value === klas)) {
    actieveKlas = klas;
    klasSelect.value = klas;
    huidigLeerjaar = leerjaarVoorKlas(klas);
    allesRenderen();
  }
};
...
function werkboekenTonen() {
  ...
  if (lbl) lbl.textContent = '(voor ' + werkboekenLabel() + ')';
...
function werkboekBestellijstTonen() {
  ...
  if (schooljaarSpan) schooljaarSpan.textContent = teamData.instellingen.schooljaar ? '(' + teamData.instellingen.schooljaar + ' - ' + werkboekenLabel() + ')' : '';
...
window.exporteren = function () {
  ...
  a.download = `bestellingen_backup_${sj}_${new Date().toISOString().split('T')[0]}.json`;
  a.click();
