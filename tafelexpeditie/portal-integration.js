...
async function loadClass(className){
 ...
 loadedClassRef=classRef;loadedCodePath=codePath;loadedCodes={...codes};loadedClassName=className;loadedAccessToken=String(data.tafelExpeditieQrToken||"");if(!loadedAccessToken){loadedAccessToken=randomToken();try{if(useLegacy)await setDoc(classRef,{tafelExpeditieQrToken:loadedAccessToken},{merge:true});else await updateDoc(classRef,{[`klassen.${className}.tafelExpeditieQrToken`]:loadedAccessToken})}catch(err){console.warn("QR-token kon niet worden bewaard",err)}}localStorage.setItem("tafelExpeditieBeheerKlas",className);window.syncPortalStudents?.(students,{schoolyear,classLabel:className});if(loadedAccessToken){const classProfiles=(window.getTafelExpeditieCloudData?.().students||[]).filter(s=>s.portalClass===className&&s.pin);for(const s of classProfiles){try{const sid=String(s.portalId||s.id),key=await studentDocId(loadedAccessToken,sid,s.pin),cloud=await getDoc(doc(firestore,"tafelexpeditie_klassen",loadedAccessToken,"leerlingen",key));if(cloud.exists())window.mergeCloudStudentProgress?.(sid,cloud.data())}catch(err){console.warn("Voortgang kon nog niet worden geladen",err)}}window.refreshTafelExpeditieTeacher?.()}await publishClassAccess().catch(err=>{console.warn("QR-klas kon nog niet worden gepubliceerd",err)})
}
