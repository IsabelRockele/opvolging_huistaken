...
const nameParts=s=>{const last=String(s.achternaam||s.lastName||s.last||"").trim(),calling=String(s.roepnaam||s.callingName||"").trim(),official=String(s.voornaam||s.firstName||s.first||"").trim(),same=(a,b)=>a&&b&&a.localeCompare(b,"nl-BE",{sensitivity:"base"})===0,first=calling&&!same(calling,last)?calling:official&&!same(official,last)?official:calling||official,direct=String(s.naam||s.name||s.fullName||s.volledigeNaam||"").trim();return{first,last,full:first&&last?`${first} ${last}`:first||last||direct}};
