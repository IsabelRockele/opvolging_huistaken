...
function addTeacherTestProfile(){
 ...
 const id=`teacher_test_${info.id}_${db.activePortalClass||"klas"}`,existing=studentById(id);if(existing){if(!existing.pin){existing.pin=makePin();saveDb();refreshAllTeacher()}alert(`${existing.name} staat al als testspeler in deze klas met code ${existing.pin}.`);return}
 const display=/^(juf|meester)\b/i.test(info.name)?info.name:`Leerkracht ${info.name}`;
 db.students.push({id,name:`${display} (test)`,firstName:display,lastName:"",pin:makePin(),isTeacherTest:true,teacherUid:info.id,createdAt:new Date().toISOString(),useCustom:false,custom:{},portalClass:db.activePortalClass||"",portalSchoolyear:"2026-2027"});saveDb();refreshAllTeacher();const added=studentById(id);alert(`${display} is toegevoegd als testspeler met code ${added.pin}. Je kunt nu ook het aanmelden aan het bord tonen.`)
}
...
function saveSettingsFromForm(){
 ...
 saveDb();$("#settingsSaved").textContent="Instellingen opgeslagen.";setTimeout(()=>$("#settingsSaved").textContent="",1800)
}
...
function printLoginLabels(){alert("De Lyreco-indeling staat klaar: 24 etiketten per A4 van 70 × 37 mm. De stickers worden afdrukbaar zodra de beveiligde klas-QR is geactiveerd, zodat er geen onbruikbare QR op de agenda's terechtkomt.")}
