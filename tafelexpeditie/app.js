...
function renderCelebrations(id){
 ...
 map.innerHTML=`<div class="expedition-title">${esc(db.activePortalClass||"Onze zebraklas")} · ${esc(data.stage)}</div><div class="expedition-progress"><i style="width:${data.pct}%"></i></div><div class="expedition-caption">Samen ${data.earned} van ${data.possible} tafelmedailles verzameld · ${data.pct}%</div>`;renderClassMedalBoard()
}
function classMedalRows(){return visibleStudents().filter(s=>!s.isTeacherTest).map(s=>({student:s,earned:earnedTables(s.id)}))}
function renderClassMedalBoard(){const box=$("#classMedalBoard");if(!box)return;const rows=classMedalRows();box.innerHTML=`<div class="class-medal-row header"><span>Kind</span>${Array.from({length:10},(_,i)=>`<span>${i+1}</span>`).join("")}<span>Totaal</span></div>${rows.map(({student,earned})=>`<div class="class-medal-row"><span class="class-medal-name">${esc(student.classNumber?student.classNumber+". ":"")}${esc(student.firstName||student.name)}</span>${Array.from({length:10},(_,i)=>`<span class="class-medal-dot ${earned.includes(i+1)?"earned":""}" title="Tafel ${i+1}">${earned.includes(i+1)?"🏅":"○"}</span>`).join("")}<span class="class-medal-total">${earned.length}/10</span></div>`).join("")||'<div class="empty-state">Nog geen kinderen in deze klas.</div>'}`}
...
function printClassMedalBoard(){const rows=classMedalRows(),name=db.activePortalClass||"Onze zebraklas",body=rows.map(({student,earned})=>`<tr><td>${esc(student.classNumber||"")}</td><td>${esc(student.firstName||student.name)}</td>${Array.from({length:10},(_,i)=>`<td class="${earned.includes(i+1)?"yes":""}">${earned.includes(i+1)?"●":"○"}</td>`).join("")}<td><b>${earned.length}/10</b></td></tr>`).join("");printDocumentHtml(`<!doctype html><html><head><style>@page{size:A4 landscape;margin:12mm}body{font-family:Arial;color:#18324a}h1{margin:0}p{color:#607483}table{width:100%;border-collapse:collapse}th,td{border:1px solid #b9c8c4;padding:7px;text-align:center}th:nth-child(2),td:nth-child(2){text-align:left}.yes{background:#fff1bb;color:#a96d00;font-size:16pt}</style></head><body><h1>${esc(name)} · klasmedaillebord</h1><p>Elke medaille betekent: de maal- én deeltafel zijn op twee toetsmomenten juist en vlot gekend.</p><table><thead><tr><th>Nr.</th><th>Naam</th>${Array.from({length:10},(_,i)=>`<th>${i+1}</th>`).join("")}<th>Totaal</th></tr></thead><tbody>${body}</tbody></table></body></html>`)}
