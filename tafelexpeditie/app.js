...
function renderStudentHome(){
 ...
 renderStudentMedals(currentStudentId)
}
...
function tableAchievement(id,table){
 ...
 const multiply=measure("multiply"),divide=measure("divide"),dates=new Set(rows.map(x=>x.date).filter(Boolean)).size,limit=db.settings.fluentSeconds*1000,earned=dates>=2&&multiply.attempts>=5&&divide.attempts>=5&&multiply.accuracy>=.9&&divide.accuracy>=.9&&multiply.avg<=limit&&divide.avg<=limit;
 const evidence=multiply.attempts+divide.attempts,almost=!earned&&dates>=1&&multiply.attempts>=3&&divide.attempts>=3&&multiply.accuracy>=.8&&divide.accuracy>=.8;
 return{earned,almost,evidence,dates,multiply,divide}
}
...
function medalState(id,t){const a=tableAchievement(id,t),m=masteryFor(id,t);if(a.earned)return["earned","🏅","behaald"];if(a.almost||m.level==="green")return["almost","★","bijna"];if(a.evidence||m.acc!=null)return["working","●","oefenen"];return["","○","start"]}
...
function renderCelebrations(id){
 const holder=$("#celebrationReadiness"),map=$("#classExpedition");if(!holder||!map)return;const level=celebrationLevel(id),student=studentById(id),data=classExpeditionData(),earned=level.earned.length?level.earned.join(", "):"nog geen",almost=level.almost.length?level.almost.join(", "):"—";
...
function printClassExpedition(){const d=classExpeditionData(),bg=new URL("assets/zebraklas-expeditie.png",location.href).href,name=db.activePortalClass||"Onze zebraklas";printDocumentHtml(`<!doctype html><html><head><style>@page{size:A4 landscape;margin:0}*{box-sizing:border-box}body{margin:0;font-family:Arial;color:#18324a}.map{position:relative;width:297mm;height:210mm;overflow:hidden}.map img{width:100%;height:100%;object-fit:cover}.copy{position:absolute;left:20mm;right:20mm;top:10mm;text-align:center}.copy h1{font-size:25pt;margin:0 0 2mm}.copy p{font-size:13pt;margin:0}.bar{height:5mm;border:1mm solid white;background:#ffffffcc;border-radius:99px;margin:4mm 30mm;overflow:hidden}.bar i{display:block;height:100%;width:${d.pct}%;background:#087f78}.footer{position:absolute;left:15mm;right:15mm;bottom:8mm;text-align:center;font-weight:bold;background:#ffffffdc;border-radius:4mm;padding:3mm}</style></head><body><section class="map"><img src="${bg}"><div class="copy"><h1>${esc(name)} · onze TafelExpeditie</h1><p>${esc(d.stage)} — samen ${d.earned} van ${d.possible} tafelmedailles</p><div class="bar"><i></i></div></div><div class="footer">Wij helpen elkaar vooruit. Iedereen groeit op zijn eigen tempo.</div></section></body></html>`)}
...
function makeHomework(id,levelChoice,play,options={}){
 ...
 if(level==="expert"){
  const familyFacts=uniqueFamilyFacts(facts,2).map(factFamily);while(familyFacts.length<2)familyFacts.push(factFamily(getF()));const a=familyFacts[0],b=familyFacts[1],items=[{prompt:`Team Zebra legt ${a.x} rijen van ${a.y} fiches. Team Kompas legt ${a.x-1||1} rijen van ${a.y} fiches. Hoeveel fiches heeft Team Zebra meer?`,steps:[`${a.x} × ${a.y} = ${a.product}`,`${Math.max(1,a.x-1)} × ${a.y} = ${Math.max(1,a.x-1)*a.y}`,`${a.product} − ${Math.max(1,a.x-1)*a.y} = ${a.product-Math.max(1,a.x-1)*a.y}`],answer:`Team Zebra heeft ${a.product-Math.max(1,a.x-1)*a.y} fiches meer.`},{prompt:`Er zijn ${b.product} vlaggetjes, eerlijk verdeeld over ${b.x} groepjes. Elk groepje krijgt daarna nog 2 vlaggetjes. Hoeveel vlaggetjes zijn er dan in totaal nodig?`,steps:[`${b.product} : ${b.x} = ${b.y}`,`${b.y} + 2 = ${b.y+2}`,`${b.x} × ${b.y+2} = ${b.x*(b.y+2)}`],answer:`Er zijn ${b.x*(b.y+2)} vlaggetjes nodig.`}];tasks.push({type:"reasoning-stories",title:`${nr++}. Expert-doordenkers`,items,level});answers.push({title:tasks.at(-1).title,items:items.map(x=>`${x.steps.join("; ")} — ${x.answer}`)})
 }
 tasks.forEach(t=>t.difficulty=level);return{id,level,tables,operation,tasks,answers}
}
function taskHtml(t){
...
function renderHomework(hw){
 ...
 const levelName=hw.level.charAt(0).toUpperCase()+hw.level.slice(1);$("#homeworkContent").innerHTML=`<div class="hw-level-chip">${levelName} · tafel${hw.tables.length>1?"s":""} ${hw.tables.join(", ")} · ${hw.operation==="multiply"?"maal":hw.operation==="divide"?"delen":"maal en delen"}</div><div class="difficulty-legend single-level"><span><i class="${hw.level}"></i> alle oefeningen op niveau ${hw.level}</span></div>`+parts.join("");
